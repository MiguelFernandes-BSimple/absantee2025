...
public class HolidayPlanServiceTest
{
    ...
    public void WhenRetrievingAllHolidayPeriodsForCollaboratorBetweenDatesThatIncludeWeekends_ThenReturnSucessfully(DateOnly searchInitDate, DateOnly searchEndDate)
    {
        ...
        Mock<IHolidayPlanRepository> repository = new Mock<IHolidayPlanRepository>();
        repository.Setup(r => r.FindAllHolidayPeriodsForCollaboratorBetweenDates(collab.Object, searchInitDate, searchEndDate)).Returns(holidayPeriodsList);

        HolidayPlanService service = new HolidayPlanService(repository.Object);
        ...
        DateOnly searchInitDate, DateOnly searchEndDate)
    {
        ...
        Mock<IHolidayPlanRepository> repository = new Mock<IHolidayPlanRepository>();
        repository.Setup(r => r.FindAllHolidayPeriodsForCollaboratorBetweenDates(collab1.Object, searchInitDate, searchEndDate)).Returns(holidayPeriodsList1);
        repository.Setup(r => r.FindAllHolidayPeriodsForCollaboratorBetweenDates(collab2.Object, searchInitDate, searchEndDate)).Returns(holidayPeriodsList2);
